...
SystemView::SystemView(SystemDefinitionSPtr sysdef)
    : sysdef{sysdef}, pdata{sysdef->getParticleData()} {
...
unsigned int SystemView::global_particle_number() const {
  return pdata->getNGlobal();
}

int SystemView::get_device_id(bool gpu_flag) const {
  maybe_unused(
      gpu_flag); // prevent compiler warnings when ENABLE_CUDA is not defined
#ifdef ENABLE_CUDA
  if (gpu_flag)

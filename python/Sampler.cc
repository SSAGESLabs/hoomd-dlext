...
template <typename> constexpr DLDataType dtype();
template <> constexpr DLDataType dtype<Scalar4>() {
  return DLDataType{kDLFloat, kBits, 1};
}
template <> constexpr DLDataType dtype<Scalar3>() {
  return DLDataType{kDLFloat, kBits, 1};
}
template <> constexpr DLDataType dtype<Scalar>() {
  return DLDataType{kDLFloat, kBits, 1};
}
template <> constexpr DLDataType dtype<int3>() {
  return DLDataType{kDLInt, 32, 1};
}
template <> constexpr DLDataType dtype<unsigned int>() {
  return DLDataType{kDLUInt, 32, 1};
}
template <> constexpr DLDataType dtype<int>() {
  return DLDataType{kDLInt, 32, 1};
}

template <typename> constexpr int64_t stride1();
template <> constexpr int64_t stride1<Scalar4>() { return 4; }
template <> constexpr int64_t stride1<Scalar3>() { return 3; }
template <> constexpr int64_t stride1<Scalar>() { return 1; }
template <> constexpr int64_t stride1<int3>() { return 3; }
template <> constexpr int64_t stride1<unsigned int>() { return 1; }

template <typename T> inline void *opaque(T *data) {
...
Sampler::Sampler(shared_ptr<SystemDefinition> sysdef,
    ...
    : HalfStepHook(), m_python_update(python_update) {
...
void Sampler::run_on_data(py::function py_exec,
  ...
  auto rtags_bridge =
      wrap<unsigned int, unsigned int>(rtags.data, on_device, 1);
  ...
  ArrayHandle<Scalar4> force(m_pdata->getNetForce(), location,
...
void Sampler::update(unsigned int timestep) {
  ...
  // Accessing the handles here holds them valid until the block of this
  // function. This keeps them valid for the python function call
#ifdef ENABLE_CUDA
  auto location = m_exec_conf->isCUDAEnabled() ? access_location::device
                                               : access_location::host;
#else
  auto location = access_location::host;
#endif // ENABLE_CUDA

  // const ArrayHandle<Scalar4> pos(m_pdata->getPositions(), location,
  // access_mode::read); auto pos_tensor = wrap<Scalar4, Scalar>(pos.data, 4 );
  // ArrayHandle<Scalar4> vel(m_pdata->getVelocities(), location,
  // access_mode::read); auto vel_tensor = wrap<Scalar4, Scalar>(vel.data, 4);
  // ArrayHandle<unsigned int> rtags(m_pdata->getRTags(), location,
  // access_mode::read); auto rtag_tensor = wrap<unsigned int, unsigned
  // int>(rtags.data, 1); ArrayHandle<int3> img(m_pdata->getImages(), location,
  // access_mode::read); auto img_tensor = wrap<int3, int>(img.data, 3);

  // ArrayHandle<Scalar4> net_forces(m_pdata->getNetForce(), location,
  // access_mode::readwrite); auto force_tensor = wrap<Scalar4,
  // Scalar>(net_forces.data, 4);

  // m_python_update(pos_tensor, vel_tensor, rtag_tensor, img_tensor,
  // force_tensor,
...
DLDataBridge Sampler::wrap(TV *ptr, const bool on_device, const int64_t size2,
                           const uint64_t offset, uint64_t stride1_offset) {
  assert((size2 >=
          1)); // assert is a macro so the extra parentheses are requiered here

  const unsigned int particle_number = this->m_pdata->getN();
#ifdef ENABLE_CUDA
  const int gpu_id =
      on_device ? m_exec_conf->getGPUIds()[0] : m_exec_conf->getRank();
#else
  ...
  bridge.tensor.dl_tensor.device =
      DLDevice{on_device ? kDLCUDA : kDLCPU, gpu_id};
  ...
  bridge.tensor.dl_tensor.dtype = dtype<TS>();
  bridge.tensor.dl_tensor.shape =
      reinterpret_cast<std::int64_t *>(bridge.shape.data());
  bridge.tensor.dl_tensor.strides =
      reinterpret_cast<std::int64_t *>(bridge.strides.data());
...
void export_Sampler(py::module &m) {
  py::class_<Sampler, std::shared_ptr<Sampler>>(m, "DLextSampler",
                                                py::base<HalfStepHook>())

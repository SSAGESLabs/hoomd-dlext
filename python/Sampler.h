...
public:
  ...
  virtual void
  setSystemDefinition(std::shared_ptr<SystemDefinition> sysdef) override;

  //! Take one timestep forward
  virtual void update(unsigned int timestep) override;

  // run a custom python function on data from hoomd
  // access_mode is ignored for forces. Forces are returned in readwrite mode
  // always.
...
private:
  ...
  DLDataBridge wrap(TS *const ptr, const bool, const int64_t size2 = 1,
                    const uint64_t offset = 0, uint64_t stride1_offset = 0);
